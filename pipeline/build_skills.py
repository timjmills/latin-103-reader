...
"""
import json, re, sys, collections, os, unicodedata

HERE = os.path.dirname(os.path.abspath(__file__))
ROOT = os.path.dirname(HERE)
OUT = os.path.join(ROOT, "app", "data", "grammar", "skills.json")
GLOSSARY = os.path.join(ROOT, "app", "data", "glossary.json")
...
skill("nominative-subject", "Nominative: the subject", NOM, "casus nominativus", "noun-case", 1, 1, [1, 2],
      ...
      "The naming form: the subject of the sentence, or a word that renames it after est; singular -a / -us / -um, plural -ae / -ī / -a.")
...
skill("genitive-possession", "Genitive: possession", GEN, "genitivus possessivus", "noun-case", 2, 2, [5],
      ...
      {"case": "gen", "pos": "N"},
      ["recognise", "chart", "parse", "blank"],
      "The 'whose?' form: -ae / -ī singular, -ārum / -ōrum plural; it can stand before or after the noun it belongs to.")
...
skill("genitive-of", "Genitive: 'of' (description, part, with adjectives)", GEN, "genitivus descriptivus et partitivus", "noun-case", 2, 2, [6, 7],
      ...
      "The genitive also does the work of English 'of': a cup of wine, a number of sons, full of apples (plēnus + genitive).")
...
skill("imperative", "Imperative: commands", "the imperative (the command form)", "modus imperativus", "verb-form", 4, 4, [13],
      ...
      ["recognise", "chart", "parse", "blank"],
      "Orders: the bare stem for one person (tacē!, venī!, pōne!), stem + -te for several (tacēte!); dīc, dūc, fac drop their final -e.")
...
skill("vocative", "Vocative: calling someone", VOC, "casus vocativus", "noun-case", 4, 4, [14],
      ...
      {"case": "voc"},
      ["recognise", "chart", "parse", "blank"],
      "The form for addressing someone: the same as the nominative except -us nouns become -e (Mārce!) and names in -ius become -ī (Iūlī!, mī fīlī!) - an ordinary noun in -ius keeps -ie (gladie!).")
...
skill("ablative-place", "Ablative: place where (in, sub + ablative)", "the ablative (the 'in/on/under' form)", "ablativus loci", "noun-case", 5, 5, [15, 16],
      ...
      {"case": "abl", "pos": "N"},
      ["recognise", "chart", "parse", "blank"],
      "Where something is: in, sub, prō take the ablative (-ā / -ō singular, -īs plural); most other prepositions take the accusative; cum is company, not place.")
...
skill("ablative-accompaniment", "Ablative: with someone (cum + ablative)", "the ablative (the 'with someone' form)", "ablativus sociativus", "noun-case", 5, 5, [15, 16],
      ...
      {"case": "abl", "pos": "N"},
      ["recognise", "parse", "blank"],
      "Who goes along: cum + ablative (cum Mārcō, cum ancillīs). Only cum does this; a thing you use takes the bare ablative with no preposition at all.")

skill("ablative-origin", "Ablative: from (ab, ex, dē + ablative)", "the ablative (the 'from' form)", "ablativus separativus", "noun-case", 5, 5, [17],
      ...
      {"case": "abl", "pos": "N"},
      ["recognise", "parse", "blank"],
      "Where something comes from or is separated from: ab, ex, dē, sine always take the ablative, whatever their sense (dē Deō = about God).")
...
skill("ablative-means", "Ablative: means (by / with a thing)", "the ablative (the 'by/with' form)", "ablativus instrumenti", "noun-case", 6, 5, [19],
      ...
      {"case": "abl", "pos": "N"},
      ["recognise", "parse", "blank"],
      "The thing used to do something goes in the ablative with no preposition (gladiō percutit = strikes with a sword); cum is only for company.")
...
skill("ablative-agent", "Ablative of agent: by a person (ā/ab + ablative)", "the ablative (the 'by' form after ā/ab)", "ablativus auctoris", "noun-case", 6, 6, [22],
      ["passive-voice", "ablative-origin"], ["ablative-means", "ablative-origin"],
      NOUN12, {"case": "abl"},
      [r"(?i)\b(a|ab)\s+\w+(a|o|is|e|ibus|u)\b"],
      {"case": "abl"},
      ["recognise", "parse", "blank"],
      "With a passive verb the person who does it takes ā/ab + ablative (ā puerō dūcitur); a thing used takes the plain ablative of means.")
...
skill("dative-indirect-object", "Dative: the indirect object", DAT, "casus dativus", "noun-case", 7, 7, [23],
      ...
      {"case": "dat"},
      ["recognise", "chart", "parse", "blank"],
      "The person something is given, said or shown to: -ae / -ō / -ī singular, -īs / -ibus plural; never marked by word order or a preposition.")
...
skill("demonstratives", "Demonstratives: hic, ille, is", "the pointing words hic ('this'), ille ('that'), is ('that / the one we mean')", "pronomina demonstrativa", "pronoun", 8, 8, [21, 24, 25],
      ...
      ["recognise", "chart", "parse", "blank"],
      "hic = this (near), ille = that (far), is = the one already mentioned; they decline like -us -a -um adjectives except genitive -īus and dative -ī.")
...
skill("demonstrative-pronouns", "Demonstratives standing alone (he, she, it)", "a demonstrative used as a pronoun (is = he, ea = she, id = it)", "pronomina demonstrativa substantiva", "pronoun", 8, 8, [26],
      ...
      ["recognise", "parse", "blank"],
      "Without a noun, hic / ille / is mean he, she, it, they; the gender follows the noun they stand for (ecce cāseus! is malus est).")

skill("relative-pronoun", "Relative pronoun: quī, quae, quod", "the relative pronoun quī, quae, quod ('who, which, that')", "pronomen relativum", "pronoun", 8, 8, [27, 28],
      ...
      {"pos": "PRON", "h": ["qui"]},
      ["recognise", "chart", "parse", "blank"],
      "quī / quae / quod opens a clause about a noun: gender and number come from that noun, the case from the pronoun's own job in its clause.")

# ---------------------------------------------------------------------------
# Familia Romana IX–XIV · Latin 101 weeks 9–14

skill("third-declension", "3rd declension nouns (masculine and feminine)", "a 3rd-declension noun (the -is / -ēs pattern: pāstor, pāstōris)", "declinatio tertia", "noun-case", 9, 9, [29],
      ...
      ["recognise", "chart", "parse", "blank"],
      "Nouns whose genitive ends in -is: the stem may change (rēx, rēg-is); endings -em, -is, -ī, -e; plural -ēs, -um / -ium, -ibus.")

skill("infinitive", "Infinitive: -re and -rī", INF, "modus infinitivus", "verb-form", 10, 10, [30, 31],
      ...
      {"mood": "inf", "tense": "pres"},
      ["recognise", "chart", "parse", "blank"],
      "The 'to' form: active -re (ambulāre, vidēre, pōnere, audīre), passive -rī / -ī (vidērī, pōnī); used after vult, potest, audet, dēbet and the like.")
...
skill("accusative-infinitive", "Accusative + infinitive (indirect statement)", "the accusative + infinitive (the 'that …' clause after says / sees / thinks)", "accusativus cum infinitivo", "syntax", 10, 10, [32, 33, 36, 37],
      ...
      {"mood": "inf"},
      ["recognise", "parse", "blank"],
      "After verbs of saying, seeing and thinking the reported subject goes into the accusative and its verb into the infinitive: videt eum aedificāre = sees that he is building.")
...
skill("third-declension-neuter", "3rd declension neuter nouns", "a 3rd-declension neuter noun (corpus, mare: nominative = accusative, plural in -a / -ia)", "declinatio tertia, genus neutrum", "noun-case", 11, 11, [34, 35],
      ...
      ["recognise", "chart", "parse", "blank"],
      "Neuter 3rd-declension nouns have identical nominative and accusative (caput, corpus), plural -a; nouns in -e / -al / -ar take -ia, -ium and ablative -ī (marī).")

skill("fourth-declension", "4th declension nouns (-us, -ūs)", "a 4th-declension noun (the u-pattern: manus, manūs)", "declinatio quarta", "noun-case", 12, 12, [38],
      ...
      {"pos": "N", "decl": 4},
      ["recognise", "chart", "parse", "blank"],
      "The u-nouns: -us, -ūs, -uī, -um, -ū; plural -ūs, -uum, -ibus; mostly masculine (manus is feminine, cornū neuter); domus mixes 4th and 2nd.")
...
skill("dative-possession", "Dative of possession (mihi est = I have)", "the dative of possession (the 'to me there is' way of saying 'I have')", "dativus possessivus", "noun-case", 12, 12, [39],
      ...
      {"case": "dat"},
      ["recognise", "parse", "blank"],
      "A dative with est says who has something: virō est gladius = the man has a sword; common with names (eī nōmen est Mārcus) and in relative clauses (cui).")
...
skill("fifth-declension", "5th declension nouns (-ēs, -eī)", "a 5th-declension noun (the e-pattern: diēs, diēī; rēs, reī)", "declinatio quinta", "noun-case", 13, 13, [42],
      ...
      {"pos": "N", "decl": 5},
      ["recognise", "chart", "parse", "blank"],
      "The e-nouns: -ēs, -eī, -eī, -em, -ē; plural -ēs, -ērum, -ēbus; diēs is masculine (feminine for a set date), rēs feminine.")
...
skill("ablative-time", "Ablative of time when", "the ablative (the 'at / in (a time)' form)", "ablativus temporis", "noun-case", 13, 13, [43],
      ...
      {"case": "abl", "pos": "N"},
      ["recognise", "parse", "blank"],
      "When something happens takes the plain ablative: nocte at night, tertiā hōrā at the third hour, eō tempore at that time; no preposition in Classical Latin.")
...
skill("present-participle", "Present participle: -ns, -ntis", "the present participle (the '-ing' word that describes a noun)", "participium praesentis", "verb-form", 14, 14, [45, 46],
      ...
      ["recognise", "chart", "parse", "blank"],
      "Stem + -ns (genitive -ntis): an adjective made from a verb, agreeing with its noun and declined like a 3rd-declension adjective (puer dormiēns, puerī dormientis).")

# ---------------------------------------------------------------------------
# Familia Romana XV–XXIV · Latin 102 weeks 1–10 (notes weeks 15–24)

skill("personal-pronouns", "Personal pronouns: ego, tū, nōs, vōs, sē", "the personal pronouns (I, you, we, you all) and the reflexive sē ('-self')", "pronomina personalia et reflexivum", "pronoun", 15, 15, [47, 48, 49],
      ...
      ["recognise", "chart", "parse", "blank"],
      "ego / tū / nōs / vōs decline in five cases (mē, mihi; tē, tibi); sē points back to the subject and has no nominative; possession uses meus, tuus, suus, noster, vester.")

skill("active-personal-endings", "Person endings: -ō, -s, -t, -mus, -tis, -nt", "the person endings (I / you / he / we / you all / they on the verb)", "desinentiae personales activae", "verb-form", 15, 15, [50, 51],
      ...
      ["recognise", "chart", "parse", "blank"],
      "The ending says who acts: -ō I, -s you, -t he/she, -mus we, -tis you all, -nt they; so ego, tū are used only for emphasis.")

skill("irregular-verbs-present", "Irregular verbs: sum, possum, eō, volō, nōlō, mālō, ferō", "the irregular verbs (be, can, go, want, not want, prefer, carry) in the present", "verba anomala", "verb-form", 15, 15, [52, 53],
      ...
      ["recognise", "chart", "parse", "blank"],
      "A few common verbs change stem (sum / es, eō / īs, vol- / vul- / vel-) or drop the linking vowel (fers, fert, vult); learn them as tables.")
...
skill("ablative-absolute", "Ablative absolute (noun + participle)", "the ablative absolute (a noun + participle set apart in the ablative: 'with X doing')", "ablativus absolutus", "syntax", 16, 16, [55],
      ...
      ["recognise", "parse", "blank"],
      "A noun and a participle both in the ablative give the circumstances of the sentence: sōle oriente = as the sun rises; nēmine vidente = with nobody watching.")

skill("ablative-degree", "Ablative of degree of difference (multō, paulō)", "the ablative of degree (the 'by how much' form: multō māior = bigger by much)", "ablativus mensurae", "noun-case", 16, 16, [56],
      ...
      {"case": "abl"},
      ["recognise", "parse", "blank"],
      "How much bigger or smaller: multō fortior = braver by much, decem pedibus altior = ten feet higher; the measure sits in the ablative next to the comparative.")

skill("passive-personal-endings", "Passive person endings: -or, -ris, -tur, -mur, -minī, -ntur", "the passive person endings (I am …-ed, you are …-ed …)", "desinentiae personales passivae", "verb-form", 17, 17, [57, 58],
      ...
      ["recognise", "chart", "parse", "blank"],
      "-or, -ris, -tur, -mur, -minī, -ntur match -ō, -s, -t, -mus, -tis, -nt; the same set serves deponent verbs (sequor, sequeris, sequitur).")

skill("adverbs", "Adverbs: -ē, -iter, and their comparison", "an adverb (the '-ly' word: how something is done) and its -ius / -issimē forms", "adverbia", "adjective", 18, 18, [59, 60],
      ...
      {"pos": "ADV"},
      ["recognise", "parse", "blank"],
      "-us -a -um adjectives make adverbs in -ē (pulchrē), 3rd-declension ones in -iter (fortiter); comparative -ius (fortius), superlative -issimē (fortissimē).")

skill("imperfect-active", "Imperfect active: -bam, -bās, -bat", IMPF, "imperfectum indicativi activi", "verb-form", 19, 19, [64, 65],
      ...
      ["recognise", "chart", "parse", "blank"],
      "Ongoing or repeated past action, and the setting of a story: stem + -bā- + person endings (ambulābat = was walking, used to walk); 3rd conj. adds -ē- (pōnēbat).")

skill("imperfect-passive", "Imperfect passive and deponent: -bātur, -bantur", "the imperfect passive (the 'was being done' form)", "imperfectum indicativi passivi", "verb-form", 19, 19, [66],
      ...
      ["recognise", "chart", "parse", "blank"],
      "Same -bā- marker with the passive endings: pulsābātur = was being hit; for deponents the meaning is active (sequēbātur = was following).")

skill("imperfect-irregular", "Imperfect of sum, possum, eō: eram, poteram, ībam", "the imperfect of the irregular verbs (eram = I was, poteram = I could, ībam = I was going)", "imperfectum verborum anomalorum", "verb-form", 19, 19, [67],
...
skill("future-active", "Future active: -bō / -am, -ēs, -et", FUT, "futurum indicativi activi", "verb-form", 20, 20, [69, 70],
      ...
      ["recognise", "chart", "parse", "blank"],
      "1st and 2nd conjugations add -b- (ambulābō, ambulābis, ambulābit); 3rd and 4th use -a- in the 'I' form and -ē- elsewhere (pōnam, pōnēs, pōnet).")

skill("future-passive", "Future passive and deponent: -bitur, -ētur", "the future passive (the 'will be done' form)", "futurum indicativi passivi", "verb-form", 20, 20, [71, 72],
      ...
      ["recognise", "chart", "parse", "blank"],
      "The same future markers with passive endings: pulsābitur = will be hit, pōnētur = will be put; deponents mean actively (sequētur = will follow).")

skill("noli-infinitive", "Negative commands: nōlī / nōlīte + infinitive", "nōlī / nōlīte + infinitive (the polite 'don't …')", "prohibitio: noli cum infinitivo", "verb-use", 20, 20, [73],
      ...
      {"mood": "imper", "h": ["nolo"]},
      ["recognise", "parse", "blank"],
      "'Do not …' is nōlī (to one person) or nōlīte (to several) plus the infinitive: nōlī venīre!, nōlīte timēre!; nē + imperative is rare.")

skill("future-irregular", "Future of sum, possum, eō: erō, poterō, ībō", "the future of the irregular verbs (erō = I will be, poterō = I will be able, ībō = I will go)", "futurum verborum anomalorum", "verb-form", 20, 20, [74],
...
skill("principal-parts", "Principal parts and the three verb stems", "the principal parts (the four dictionary forms that give a verb's present, perfect and participle stems)", "partes principales verbi", "vocabulary", 21, 21, [75, 76, 77],
      ...
      {"pos": "V"},
      ["recognise", "blank"],
      "agō, agere, ēgī, āctum: the infinitive gives the present stem, the third part the perfect stem (ēg-), the fourth the participle stem (āct-); 1st, 2nd and 4th conjugations are predictable.")

skill("perfect-active", "Perfect active: -ī, -istī, -it, -ērunt", PERF, "perfectum indicativi activi", "verb-form", 21, 21, [78, 79],
      ...
      ["recognise", "chart", "parse", "blank"],
      "Completed action or plain past ('did', 'has done'): the perfect stem + -ī, -istī, -it, -imus, -istis, -ērunt (vēnit = he came / has come; vēnērunt = they came).")

skill("perfect-passive-participle", "Perfect passive participle: -tus, -sus", "the perfect participle (the 'having been done' word: laudātus = praised)", "participium perfecti passivi", "verb-form", 21, 21, [80, 81],
      ...
      ["recognise", "chart", "parse", "blank"],
      "The fourth principal part as an -us -a -um adjective: puer laudātus = the boy (having been) praised; it agrees with its noun like any adjective.")

skill("perfect-passive", "Perfect passive: laudātus est", "the perfect passive (the 'was done / has been done' form: laudātus est)", "perfectum indicativi passivi", "verb-form", 21, 21, [82],
      ...
      {"mood": "ptc", "tense": "perf", "voice": "pass"},
      ["recognise", "chart", "parse", "blank"],
      "Perfect participle + present of sum: vīsus est = he was seen / has been seen; the participle agrees with the subject (vīsī sunt), and est translates as 'was'.")
...
skill("perfect-infinitive", "Perfect infinitive: -isse, -um esse", "the perfect infinitive (the 'to have done' form)", "infinitivus perfecti", "verb-form", 21, 21, [83],
      ...
      {"mood": "inf", "tense": "perf"},
      ["recognise", "chart", "parse", "blank"],
      "Active: perfect stem + -isse (fēcisse = to have done); passive: participle + esse (factum esse); used in indirect statement for an earlier action (scit eum vēnisse).")

skill("supine", "Supine: -um after motion, -ū with adjectives", "the supine (the special purpose form after a verb of going: dormītum it = goes to sleep)", "supinum", "verb-form", 22, 22, [84],
      ...
      {"mood": "supine"},
      ["recognise", "chart", "parse", "blank"],
      "The participle stem + -um after a verb of motion says why one goes (vēnātum eō = I go to hunt); the -ū form completes an adjective (facile dictū = easy to say).")
...
skill("ablative-absolute-perfect", "Ablative absolute with a perfect participle", "the ablative absolute with a perfect participle ('with X done', 'after X was done')", "ablativus absolutus cum participio perfecti", "syntax", 22, 22, [85],
      ...
      ["recognise", "parse", "blank"],
      "Noun + perfect participle in the ablative for something already done when the main action happens: hōc dictō = this said; cibō parātō = the food prepared.")

skill("perfect-deponent-participle", "Perfect participle of deponents: 'having done'", "the perfect participle of a deponent verb (active meaning: locūtus = having spoken)", "participium perfecti verbi deponentis", "verb-form", 22, 22, [86],
      ...
      ["recognise", "parse", "blank"],
      "Deponents form the perfect participle like other verbs but it means 'having done', not 'having been done': puer lāpsus = the boy having slipped; fēmina profecta = the woman having set out.")

skill("perfect-deponent", "Perfect of deponents: locūtus est", "the perfect of a deponent verb (locūtus est = he spoke)", "perfectum verbi deponentis", "verb-form", 22, 22, [87],
      ...
      {"mood": "ptc", "tense": "perf", "deponent": True},
      ["recognise", "chart", "parse", "blank"],
      "Participle + sum, translated actively: secūtus est = he followed, profectī sunt = they set out; it looks like a perfect passive and never is.")

skill("future-participle", "Future participle: -ūrus, 'about to'", "the future participle (the 'about to' word: ventūrus = about to come)", "participium futuri activi", "verb-form", 23, 23, [88],
      ...
      ["recognise", "chart", "parse", "blank"],
      "Participle stem + -ūr- + -us -a -um: puer ambulātūrus = a boy about to walk, mīlitēs moritūrī = soldiers about to die; futūrus = about to be.")

skill("future-infinitive", "Future infinitive: -ūrum esse", "the future infinitive (the 'going to' form: ventūrum esse)", "infinitivus futuri", "verb-form", 23, 23, [89],
      ...
      {"mood": "inf", "tense": "fut"},
      ["recognise", "chart", "parse", "blank"],
      "Future participle + esse (often left out) in indirect statement for what will happen: dīcit sē ventūrum esse = says that he will come; sum has fore or futūrum esse.")
...
skill("pluperfect", "Pluperfect: -eram, -erat; -us erat", PLUPF, "plusquamperfectum indicativi", "verb-form", 24, 24, [90, 91],
      ...
      ["recognise", "chart", "parse", "blank"],
      "Something finished before another past event: perfect stem + -erā- + endings (vēnerat = he had come); passive and deponent use the participle + erat (captus erat, profectī erant).")

skill("ablative-comparison", "Ablative of comparison (than, without quam)", "the ablative of comparison (the 'than' form: illō fortior = braver than he)", "ablativus comparationis", "noun-case", 24, 24, [92],
      ...
      {"case": "abl"},
      ["recognise", "parse", "blank"],
      "Instead of quam + the same case, the thing compared can stand alone in the ablative: hic illō fortior est = this one is braver than that one; nihil hōc pēius = nothing worse than this.")

# ---------------------------------------------------------------------------
# Familia Romana XXV–XXXIV · Latin 103 weeks 1–14

skill("deponent-imperatives", "Deponent imperatives: -re, -minī", "the deponent command forms (sequere! = follow!, sequiminī! = follow, all of you!)", "imperativus verborum deponentium", "verb-form", 25, 25, [93],
      ...
      ["recognise", "chart", "parse", "blank"],
      "To one person the deponent imperative looks like an active infinitive (sequere!, intuēre!); to several it ends in -minī (sequiminī!, laetāminī!).", syl=1)

skill("gerund", "Gerund: the verb as a noun (-ndum, -ndī, -ndō)", "the gerund (the '-ing' noun form: ad bibendum = for drinking)", "gerundium", "verb-form", 26, 26, [94, 95],
      ...
      {"mood": "gerund"},
      ["recognise", "chart", "parse", "blank"],
      "Stem + -nd- with neuter endings: ad + -ndum for purpose (ad nandum), -ndī after a noun or with causā (ars scrībendī), -ndō for means or after in / dē (in ambulandō).", syl=2)
...
skill("present-subjunctive", "Present subjunctive: forms (-em, -am; sim, possim, velim)", "the present subjunctive (the 'may / let' form: veniat = let him come, may he come)", "coniunctivus praesentis", "verb-form", 27, 27, [96, 99, 100],
      ...
      ["recognise", "chart", "parse", "blank"],
      "Change the stem vowel: 1st conjugation a → e (ambulet), the others add -ā- (videat, pōnat, audiat); sum, possum, volō take -ī- (sit, possit, velit), eō has eat.", syl=3)

skill("subjunctive-wish-command", "Subjunctive on its own: wishes, 'let us', 'let him'", "the subjunctive as a wish or gentle command (eāmus = let's go; veniat = let him come)", "coniunctivus optativus, hortativus, iussivus", "verb-use", 27, 27, [96, 97],
      ...
      {"tense": "pres", "mood": "subj"},
      ["recognise", "parse", "blank"],
      "In a main clause the present subjunctive expresses what the speaker wants: a wish (deī tē servent), a suggestion (discēdāmus, let us leave), an order to a third person (veniant); negative nē.", syl=3)
...
skill("potential-subjunctive", "Potential subjunctive: 'could', 'would'", "the potential subjunctive (the 'could / would / may' form: velim = I would like)", "coniunctivus potentialis", "verb-use", 27, 27, [98, 100],
      ...
      ["recognise", "parse", "blank"],
      "The subjunctive can soften a statement to a possibility: tē adiuvem = I could help you; aliquis dīcat = someone might say; velim, nōlim, mālim = I would like / rather not / prefer.", syl=4)

skill("indirect-command", "Indirect command: ut / nē + subjunctive", "ut / nē + subjunctive (the form for what someone asks or orders someone to do)", "coniunctivus substantivus: ut, ne", "syntax", 27, 27, [101, 102],
      ...
      ["recognise", "parse", "blank"],
      "After verbs of ordering, asking, persuading and wishing (imperat, ōrat, rogat, monet, vult) the thing wanted goes in ut / nē + subjunctive: imperat ut veniant; ōrat nē discēdat. iubeō instead takes the infinitive.", syl=4)

skill("purpose-clause", "Purpose clause: ut / nē + subjunctive ('in order to')", "ut / nē + subjunctive (the 'in order to' clause: venit ut videat = comes to see)", "coniunctivus finalis", "syntax", 28, 28, [103],
      ...
      {"mood": "subj", "tense": ["pres", "impf"]},
      ["recognise", "parse", "blank"],
      "ut + subjunctive says why something is done (veniunt ut cēnent = they come in order to dine), nē for a negative aim; a relative quī + subjunctive can do the same job (mittit quī dīcat).", syl=5)
...
skill("result-clause", "Result clause: tam / ita … ut ('so … that')", "ut + subjunctive after tam / ita / tantus (the 'so … that' clause)", "coniunctivus consecutivus", "syntax", 28, 28, [104],
      ...
      {"mood": "subj", "tense": ["pres", "impf"]},
      ["recognise", "parse", "blank"],
      "A signal word (tam so, tantus so big, tālis of such a kind, ita in such a way, tot so many) is answered by ut + subjunctive telling what follows; the negative is ut nōn, never nē.", syl=5)

skill("imperfect-subjunctive", "Imperfect subjunctive: forms (infinitive + endings)", "the imperfect subjunctive (the past-time 'might / would' form: venīret; built on the infinitive)", "coniunctivus imperfecti", "verb-form", 28, 28, [105, 107],
      ...
      ["recognise", "chart", "parse", "blank"],
      "The present active infinitive plus person endings: ambulāre-t, vidēre-t, pōnere-t, audīre-t; passive pōnere-tur; irregulars the same way (esse-t, posse-t, velle-t, ferre-t).", syl=5)
...
skill("sequence-of-tenses", "Sequence of tenses: present vs imperfect subjunctive", "the sequence of tenses (a present main verb takes the present subjunctive, a past one the imperfect)", "consecutio temporum", "verb-use", 28, 28, [105, 106],
      ...
      {"mood": "subj", "tense": ["pres", "impf"]},
      ["recognise", "parse", "blank"],
      "Look at the main verb: present or future → present subjunctive (imperat ut veniās); any past tense → imperfect subjunctive (imperāvit ut venīrēs). The same holds for purpose, result, cum and indirect questions.", syl=6)
...
skill("cum-narrative", "cum + subjunctive: 'when / while' in a story", "cum + subjunctive (the 'when / while' clause that sets the scene in a past story)", "cum historicum", "syntax", 28, 28, [108],
      ...
      ["recognise", "parse", "blank"],
      "In past narrative cum + imperfect subjunctive = 'while / as' (cum ambulāret, latrōnēs vīdit), cum + pluperfect = 'after / when he had' (cum dīxisset, abiit); cum + ablative is 'with', cum + indicative a plain date.", syl=6)
...
skill("cum-causal", "cum + subjunctive: 'since', 'although'", "cum + subjunctive meaning 'since' (or, with tamen, 'although')", "cum causale et concessivum", "syntax", 29, 29, [109],
      ...
      {"mood": "subj"},
      ["recognise", "parse", "blank"],
      "cum + subjunctive can give the reason (plōrō cum mortua sit = I weep since she is dead) or, with tamen in the main clause, a concession (cum dīves sit, tamen …); quippe quī + subjunctive = 'as one who'.", syl=7)

skill("indirect-question", "Indirect question: quis / cūr / ubi + subjunctive", "an indirect question (a question folded into a sentence: nesciō quid faciat = I don't know what he is doing)", "interrogatio obliqua", "syntax", 29, 29, [110],
      ...
      {"mood": "subj"},
      ["recognise", "parse", "blank"],
      "After ask, know, tell, wonder, a question word (quis, quid, cūr, ubi, quōmodo, num, utrum … an) introduces a clause whose verb is subjunctive: sciunt ubi sīs; nesciēbam quid vidēret.", syl=7)
...
skill("deliberative-subjunctive", "Deliberative subjunctive: 'what am I to do?'", "the deliberative subjunctive (the 'what am I to do?' question: quid faciam?)", "coniunctivus deliberativus", "verb-use", 29, 29, [111],
      ...
      ["recognise", "parse", "blank"],
      "A doubtful or despairing question in the subjunctive, usually first person: quid faciam? what am I to do?; quō eāmus? where are we to go?; past time uses the imperfect (quid facerem?).", syl=7)

skill("future-perfect", "Future perfect: -erō, -erit ('will have done')", FUTPERF, "futurum exactum", "verb-form", 30, 30, [112, 113],
      ...
      ["recognise", "chart", "parse", "blank"],
      "Perfect stem + -erō, -eris, -erit, -erimus, -eritis, -erint for an action finished before a future one: cum vēnerō, gaudēbō = when I have come, I'll be glad; passive: captus erit.", syl=8)
...
skill("gerundive", "Gerundive: the -ndus adjective", "the gerundive (the '-nd-' adjective: 'to be done'; aqua bibenda = water to be drunk)", "gerundivum (participium futuri passivi)", "verb-form", 31, 31, [114, 120],
      ...
      {"mood": "gerundive"},
      ["recognise", "chart", "parse", "blank"],
      "Stem + -nd- + -us -a -um, agreeing with a noun: what should be done to it (liber legendus); after ad or with causā it replaces gerund + object (ad litterās scrībendās = for writing letters).", syl=9)
...
skill("dative-of-agent", "Dative of agent with the gerundive (mihi = by me)", "the dative of agent (the person who must do it: mihi legendum est = I must read)", "dativus auctoris", "noun-case", 31, 31, [114],
      ...
      {"case": "dat"},
      ["recognise", "parse", "blank"],
      "With the gerundive + sum the doer goes in the dative, not ā/ab + ablative: liber mihi legendus est = the book must be read by me, I have to read the book; servīs labōrandum est.", syl=9)

skill("perfect-subjunctive", "Perfect subjunctive: -erim, -erīs, -erit", "the perfect subjunctive (the 'have done' form: nesciō quid fēcerit; nē timuerīs = don't be afraid)", "coniunctivus perfecti", "verb-form", 32, 32, [115],
      ...
      ["recognise", "chart", "parse", "blank"],
      "Perfect stem + -eri- + endings (fēcerim, fēcerīs, fēcerit …; passive factus sim) for a completed action after a present main verb: nesciō quis vēnerit; cum clauserit; and nē + 2nd person for 'don't' (nē timuerīs). -erit and -erint also spell the future perfect, whose -eris is short where this -erīs is long.", syl=10)
...
skill("wishes-utinam", "Wishes: utinam + subjunctive", "utinam + subjunctive (the 'if only …' wish; the tense says whether it can still come true)", "coniunctivus optativus cum utinam", "verb-use", 32, 33, [118],
      ...
      {"mood": "subj"},
      ["recognise", "parse", "blank"],
      "utinam (negative utinam nē) + present subjunctive = a wish that may still happen (utinam veniat); + imperfect = a present wish that cannot (utinam venīrent, if only they were coming); + pluperfect = a past one (utinam vēnisset).", syl=11)

skill("pluperfect-subjunctive", "Pluperfect subjunctive: -issem, -isset", "the pluperfect subjunctive (the 'had / would have' form: cum vēnisset = when he had come)", "coniunctivus plusquamperfecti", "verb-form", 33, 33, [116, 117],
      ...
      ["recognise", "chart", "parse", "blank"],
      "The perfect infinitive + endings (fēcisse-m, fēcisse-t; passive factus essem) for an action completed before a past main verb: cum advēnissent, cibum sūmpsērunt; nesciēbam quis advēnisset.", syl=12)

skill("conditions-contrary-to-fact", "Unreal conditions: sī + imperfect / pluperfect subjunctive", "the contrary-to-fact condition (sī + subjunctive: 'if I were …, I would …'; 'if I had …, I would have …')", "condicio irrealis", "syntax", 33, 33, [119],
      ...
      ["recognise", "parse", "blank"],
      "Imperfect subjunctive in both halves for an unreal present (sī thunnus essem, in marī habitārem = if I were a tuna I would live in the sea); pluperfect for an unreal past (sī bibisset, nōn mortuus esset); tamquam takes the same tenses.", syl=12)

skill("future-imperative", "Future imperative: estō, mementō, -tō", "the future imperative (the 'you shall …' order for later or for rules: estō! mementō!)", "imperativus futuri", "verb-form", 33, 33, [],
      ...
      ["recognise", "chart", "parse", "blank"],
      "Stem + -tō (plural -tōte, 3rd person -ntō) gives an order for the future or a standing rule: estō = be (from now on), mementō = remember, scītō = know that; common in laws and maxims.", syl=12)

skill("dative-verbs", "Verbs that take the dative (pāreō, placeō, noceō, crēdō)", DAT, "verba cum dativo", "noun-case", 34, 34, [],
      ...
      {"case": "dat"},
      ["recognise", "parse", "blank"],
      "Some verbs put their object in the dative where English has a direct object: pāreō obey, placeō please, noceō harm, crēdō believe, persuādeō persuade, ignōscō forgive, and compounds of sum (adsum, prōsum): servī dominō pārent.", syl=13)
...
skill("dummodo", "dummodo + subjunctive: 'provided that'", "dummodo + subjunctive (the 'provided that / as long as' condition)", "dummodo cum coniunctivo", "syntax", 34, 34, [],
      ...
      {"mood": "subj", "tense": ["pres", "impf"]},
      ["recognise", "parse", "blank"],
      "dummodo (also modo, dum) + subjunctive states a condition that must hold: ōderint dum metuant = let them hate, provided they fear; negative dummodo nē.", syl=13)
...
skill("prosody-scansion", "Prosody: long and short syllables, elision", "prosody (which syllables count long or short, and when a final vowel is swallowed by the next word)", "prosodia: quantitas syllabarum, elisio", "metre", 34, 34, [],
      ...
      None,
      ["recognise"],
      "A syllable is long if its vowel is long, a diphthong, or followed by two consonants; a final vowel or -m before a word starting with a vowel or h- is elided (ōd(ī) et amō); the hexameter pauses at a caesura.", syl=14)
...
FUNCTION = {
    ...
    "ablative-origin": "from, out of, away from (ā / ab / ē / ex / dē + ablative)",
    "accusative-destination": "motion towards (ad / in + accusative)",
    "ablative-means": "means (with a thing, no preposition)",
    "ablative-agent": "agent (by a person: ā / ab + ablative)",
    ...
    "noli-infinitive": "negative command (nōlī + infinitive)",
    "perfect-passive": "perfect passive (laudātus est = was praised)",
    "ablative-absolute-perfect": "ablative absolute (noun + perfect participle)",
    "perfect-deponent": "perfect of a deponent (locūtus est = he spoke)",
    ...
    "dative-verbs": "object of a dative verb (pāreō, placeō, noceō, crēdō …)",
...
FUNCTION_KEYS = {
    ...
    "noli-infinitive": ["negative command", "prohibition", "nōlī"],
...
# ---------------------------------------------------------------------------
# Macrons in the learner-facing strings (QA-FINAL B5).
#
# `summary` is the most-repeated line in the whole section: the rule under the
# skill title on the map, the entire Hint of every guided item, the "Why" of
# every feedback panel and the footer of the printed chart.  `title`, `plain`,
# `latin_label`, `function` and `function_keys` are printed nearly as widely.
# A Latin word or ending in any of them therefore has to be spelled as Orberg
# prints it, or the chart teaches `-ae / -o / -i` one line above the lesson's
# `-ae / -ō / -ī`, and the perfect gets illustrated by `venit`, which without
# its macron is the present.
#
# The authority is latin_forms.py over app/data/glossary.json — both committed,
# so the check is deterministic and needs no private corpus.  A word is wrong
# when *no* legal spelling of it is at least as macronised as the string: i.e.
# every form latin_forms generates with those letters carries a macron the
# string does not.  Endings (-ae, -ō, -ērunt) are checked against the ending
# tables themselves.  A marker written between two hyphens (-bā-, -eri-, -nd-)
# is a stem formant, not an ending, and is left to the author.

# English words that happen to spell a Latin form latin_forms generates
# (`place` = placē, imperative of placēre; `time` = timē; `i` = ī, go!).  Add a
# word here only when a new learner-facing string genuinely needs the English
# one; never to silence a Latin word that is missing its macron.
ENGLISH_HOMOGRAPHS = {
    "also", "are", "caesura", "case", "comes", "do", "future", "has", "have",
    "i", "line", "me", "means", "no", "persuade", "place", "serves", "time", "use",
}

# Latin words whose correct spelling really has no macron, where the same
# letters also spell a macronised form the generator knows: the adverb `modo`
# ("only, just") against the ablative `modō`.  Each entry is a word the book
# prints short; check the printed page before adding one.
SHORT_LATIN = {"modo"}

LEARNER_FACING = ("title", "plain", "latin_label", "summary", "function")

_COMBINING = re.compile(r"[̀-ͯ]")
_TOKEN = re.compile(r"(-?)([A-Za-zĀāĒēĪīŌōŪūȲȳ]+)(-?)")


def _bare(s):
    """The word with every macron taken off, lower-cased."""
    return _COMBINING.sub("", unicodedata.normalize("NFD", s)).lower()


def _macron_at(s):
    """Indexes, counted over the macron-free letters, that carry a macron."""
    out, i = set(), 0
...
def macron_authority():
    """(words, endings): bare spelling -> every macronised form latin_forms knows.

    Words come from running the form generator over every glossary entry;
    endings from the ending tables it declines with.  Raises if either input is
    missing — a check that cannot run must fail the build, not pass it.
    """
    if HERE not in sys.path:
        sys.path.insert(0, HERE)
    import latin_forms as lf              # noqa: E402  (needs sys.path above)
    ...
    for entries in glossary.values():
        for e in entries:
            key = (e.get("lemma"), e.get("h"), e.get("pos"))
            if key in seen:
                continue
            seen.add(key)
            try:
                generated = lf.single_forms(e)
            except Exception:
                continue                    # a malformed entry is build_glossary's problem
            for form in generated:
                words[_bare(form)].add(form.lower())

    raw = set()
    for table in lf.NOUN_ENDINGS.values():
        for number in ("sg", "pl"):
            raw.update(table.get(number) or [])
    for table in (lf.ADJ_COMP, lf.ADJ_PTC):
        for number in ("sg", "pl"):
            for row in table[number]:
                raw.update(row)
    for conj in lf.CONJ.values():
        for value in conj.values():
            if isinstance(value, dict):
                for cells in value.values():
                    raw.update(cells) if isinstance(cells, list) else raw.add(cells)
            elif isinstance(value, str):
                raw.add(value)
    for table in (lf.PERF, lf.SUM_FORMS):
    ...
    for ending in raw:
        endings[_bare(ending)].add(ending.lower())
    return words, endings


def unmacronised(text, words, endings):
    """[(token, what latin_forms spells it), ...] for every macron missing in `text`."""
    wrong = []
    for m in _TOKEN.finditer(text):
        lead, word, trail = m.group(1), m.group(2).lower(), m.group(3)
        if lead and trail:
            continue                        # -bā-, -eri-: a stem formant, not an ending
        if lead:
            known = endings.get(_bare(word))
        else:
            if word in ENGLISH_HOMOGRAPHS or word in SHORT_LATIN:
                continue
            known = words.get(_bare(word))
        if not known:
            continue                        # not a form latin_forms can rule on
        here = _macron_at(word)
        if any(_macron_at(k) <= here for k in known):
            continue                        # some legal spelling is no more macronised
        dash = "-" if lead else ""
        wrong.append((dash + word, " / ".join(dash + k for k in sorted(known)), bool(lead)))
    return wrong


def check_macrons(doc):
    words, endings = macron_authority()
    errs = []
    for s in doc["skills"]:
        strings = [(f, s.get(f)) for f in LEARNER_FACING]
        strings += [("function_keys", k) for k in (s.get("function_keys") or [])]
        for field, text in strings:
            if not text:
                continue
            for token, spelling, is_ending in unmacronised(text, words, endings):
                hint = ("the ending tables have no short one" if is_ending else
                        "if the word is English, add it to ENGLISH_HOMOGRAPHS; "
                        "if the book really prints it short, to SHORT_LATIN")
                errs.append("%s: %s prints %s where latin_forms has %s — %s"
                            % (s["id"], field, token, spelling, hint))
    return errs
...
def validate(doc):
    ...
    # Every skill must have a hand-macronised Latin name: annotate() only
    # overwrites the `skill(...)` argument where LATIN has one, so a skill
    # missing from LATIN would ship its unmacronised placeholder.
    for i in sorted(idset - set(LATIN)):
        errs.append(i + ": no macronised name in LATIN")
    errs += check_macrons(doc)
    return errs
...
def main():
    # The macron errors below quote Latin, so say so rather than dying on a
    # cp1252 console when PYTHONIOENCODING is not set.
