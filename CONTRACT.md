...
Reviews (2026-09-05): `reading_progress.reads` (default 1) and `last_read_at`. When a sentence that already has a row meets the "read" rule again in a *later session* (≥ 30 min after `last_read_at`), `reads` increments and `last_read_at` updates; `read_at` never changes. Pace, "time left" and "N of M read" use first reads only. The study log shows reviews separately ("14 read · 58 reviewed" per day; per week "93 read · 2 passes" = max reads). Reset progress clears everything for the week; there is no reset for reviews alone.
