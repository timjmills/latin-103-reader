...
`store.getProgress()` → `Map<unit_id, read_at>` (first reads: counts, Continue, the weeks menu); `store.getProgressRows()` → `Map<unit_id, { unit_id, week_n, read_at, reads, last_read_at }>` (the reader's timers and the study log's review figures); `store.markRead(unitIds[])` (splits first reads from reviews locally, see "Reviews" below); `store.resetProgress(weekN|null)` (one week or everything, after a confirm). Lookups are never touched by progress or by any reseed.
...
Reviews (2026-09-05): `reading_progress.reads` (default 1) and `last_read_at`. When a sentence that already has a row meets the "read" rule again in a *later session* (≥ 30 min after `last_read_at`), `reads` increments and `last_read_at` updates; `read_at` never changes; within 30 min nothing changes. Wherever two copies of a row meet (a pull, realtime, the upsert on the server) they merge field by field: `reads` = max, `last_read_at` = max, `read_at` = min. Pace, "time left" and "N of M read" use first reads only. The study log shows reviews separately ("14 read · 58 reviewed" per day — a sentence counts once per day, on the day of its `last_read_at`; per week "93 read · 2 passes" = max reads); sentence view's tick reads "read · reviewed" / "read · reviewed ×2". Reset progress clears everything for the week, reviews included; there is no reset for reviews alone.
Study log merge (2026-09-05, replaces "additive-max"): `study_days` rows are per (day, device); each device keeps its own running total (`device` = a random id kept in localStorage, `main` for the backfilled baseline); the server keeps the max per row; the client sums the devices' rows per day on read. Pace counts only days with first reads.

## Book lines (2026-09-05)

`unit.lines` — `[{ "line": 12, "start": 0 }, { "line": 13, "start": 41 }]`: where each printed book line begins inside `la` (character offset; the first entry is the line the sentence starts on, start 0). Empty for Fabellae Latinae and any unit the scan could not map. Produced by `pipeline/attach_margins.py` from `lines-week-NN.json`; column `units.lines` (migration 0013); passed through `store.getUnits`.
Setting `lineMode: 'flow' | 'book'` (default flow). Book mode, passage view only: the text is laid out one printed line per screen line (a break at every `start`), every line numbered in the gutter (not just block starts), sentences still tappable and highlighted as before, margin notes aligned to their line. Weeks/parts without line data show flow layout and the toggle explains why.
